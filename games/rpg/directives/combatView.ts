...
module rpg.directives {
  ...
  export class CombatViewController implements pow2.IProcessObject {
    ...
    choosing:rpg.objects.GameEntityObject = null;
    ...
    setPointerTarget(object:rpg.objects.GameEntityObject, directionClass:string = "right", offset:pow2.Point = new pow2.Point()) {
      var el:JQuery = $(this.pointer.element);
    ...
    getTargets():rpg.objects.GameEntityObject[] {
      var result:rpg.objects.GameEntityObject[] = [];
    ...
    return {
      ...
      link: (scope, element, attrs, controller:CombatViewController) => {
        ...
        var chooseTurns = (data:rpg.states.combat.IChooseActionEvent) => {
          ...
          var choices:rpg.objects.GameEntityObject[] = data.players.slice();
          var next = () => {
            var p:rpg.objects.GameEntityObject = choices.shift();
            if (!p) {

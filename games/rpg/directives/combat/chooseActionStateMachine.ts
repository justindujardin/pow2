...
module rpg.directives.combat {
  ...
  export class ChooseActionStateMachine extends pow2.StateMachine {
    current:rpg.objects.GameEntityObject = null;
    target:rpg.objects.GameEntityObject = null;
    scene:pow2.scene.Scene;
    player:pow2.game.components.PlayerCombatRenderComponent = null;
    action:rpg.components.combat.CombatActionComponent = null;
    spell:any = null;
    scope:any;
    world:rpg.GameWorld = rpg.GameWorld.get();

    constructor(public controller:CombatViewController,
                public data:rpg.states.combat.IChooseActionEvent,
      ...
      this.scene = <pow2.scene.Scene>data.players[0].scene;
  ...
  export class ChooseActionType extends pow2.State {
    ...
    enter(machine:ChooseActionStateMachine) {
      ...
      var p:rpg.objects.GameEntityObject = machine.current;
      ...
      var selectAction = (action:rpg.states.IPlayerAction) => {
        machine.action = <rpg.components.combat.CombatActionComponent>action;
  ...
  export class ChooseActionTarget extends pow2.State {
    ...
    enter(machine:ChooseActionStateMachine) {
      ...
      var enemies = <rpg.objects.GameEntityObject[]>machine.data.enemies;

      var p:rpg.objects.GameEntityObject = machine.target || enemies[0];
      var el:JQuery = $(machine.controller.pointer.element);
      ...
      var selectTarget = (target:rpg.objects.GameEntityObject) => {
  ...
  export class ChooseActionSubmit extends pow2.State {
    ...
    constructor(public submit:(action:rpg.components.combat.CombatActionComponent)=>any) {
      super();
    }

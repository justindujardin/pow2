...
module rpg.states {
  ...
  export class CombatStateMachine extends pow2.StateMachine {
    ...
    isFriendlyTurn():boolean {
      return this.current && !!_.find(this.party, (h:rpg.objects.GameEntityObject) => {
            return h._uid === this.current._uid;
          });
    }

    getLiveParty():rpg.objects.GameEntityObject[] {
      return _.reject(this.party, (obj:rpg.objects.GameEntityObject) => {
        return obj.isDefeated();
      });
    }

    getLiveEnemies():rpg.objects.GameEntityObject[] {
      return _.reject(this.enemies, (obj:rpg.objects.GameEntityObject) => {
        return obj.isDefeated();
      });
    }

    getRandomPartyMember():rpg.objects.GameEntityObject {
      var players = <rpg.objects.GameEntityObject[]>_.shuffle(this.party);
    ...
    getRandomEnemy():rpg.objects.GameEntityObject {
      var players = <rpg.objects.GameEntityObject[]>_.shuffle(this.enemies);
      while (players.length > 0) {
        var p = players.shift();
        if (!p.isDefeated()) {
          return p;
    ...
    partyDefeated():boolean {
      var deadList = _.reject(this.party, (obj:rpg.objects.GameEntityObject) => {
  ...
  export class GameCombatState extends pow2.State {
    ...
    enter(machine:GameStateMachine) {
      ...
      _.each(machine.model.party, (hero:rpg.models.HeroModel, index:number) => {
        var heroEntity:rpg.objects.GameEntityObject = this.factory.createObject('CombatPlayer', {
          model: hero,
          combat: this
        });
        if (!heroEntity) {
      ...
      machine.world.loader.load(mapUrl, (map:pow2.TiledTMXResource)=> {
        ...
        for (var i:number = 0; i < enemiesLength; i++) {
          ...
          var nme:rpg.objects.GameEntityObject = this.factory.createObject('CombatEnemy', {
            model: nmeModel,
            combat: this,
            sprite: {
              name: "enemy",
        ...
        if (this.machine.enemies.length) {
          _.each(this.machine.party, (heroEntity:rpg.objects.GameEntityObject, index:number) => {
            var battleSpawn = this.tileMap.getFeature('p' + (index + 1));
            heroEntity.setPoint(new pow2.Point(battleSpawn.x / 16, battleSpawn.y / 16));
          });

          _.each(this.machine.enemies, (enemyEntity:rpg.objects.GameEntityObject, index:number) => {
